 ...
 *
 * v7 — realign fitness to excess-Sortino-vs-buy-hold (Path B).
 *
 * v6 exposed: mean val Calmar across 40 seeds was only 0.33. Cherry-picking
 * best-of-40 gets Calmar > 1.0 by luck; holdout confirmed the bot has no real
 * edge (holdout Calmar -1.054, CAGR -13.8%).
 *
 * Diagnosis: bot optimizes Calmar (return/DD) but members' alternative is
 * hold BTC in a wallet with zero fees. Bot must beat NOT cash, but buy-hold
 * on risk-adjusted basis.
 *
 * NEW FITNESS: SORTINO of daily excess returns vs buy-hold.
 *   excess_daily[t] = bot_return[t] - btc_return[t]
 *   sortino_of_excess = mean(excess_daily) / downside_stdev(excess_daily) * sqrt(365)
 *
 * Rewards:
 *   Bot flat while BTC drops -> positive excess -> Sortino up
 *   Bot rides bulls fully    -> zero excess -> Sortino unchanged
 *   Bot misses bulls         -> negative excess -> Sortino DOWN (bull-miss punished)
 *   Bot loses when BTC didn't -> negative excess -> Sortino DOWN
 *
 * v7 changes vs v6-A:
 *   - Fitness: geo-mean(excess-Sortino per fold) instead of geo-mean(Calmar per fold)
 *   - Bull-capture penalty REMOVED (redundant with new fitness)
 *   - Same 17g A-shape, same realism, same 4-fold K-split, same watchdog
 *   - Single arm this round (no macro dropped: 17g)
 *
 * CLI: npx tsx scripts/11-evolve-v7.ts <runId> <minutesPerSeed> [nSeeds]
 */
...
interface BotResult {
  ...
  bull_capture_ratio: number;
  excess_sortino: number;      // v7: annualized Sortino of daily excess returns vs buy-hold
  excess_mean_daily: number;   // v7: mean daily excess return (bot - BH)
  buy_hold_cagr: number;       // v7: BH CAGR for direct comparison
}
...
function simulate(bars: BtcDay[], indic: { rsi: number[]; vol20: number[] }, eventStates: EventState[], macroStates: MacroState[], genome: Genome, startIdx: number, endIdx: number): BotResult {
  ...
  // v6 bull-capture (kept for reporting, not used in v7 fitness)
  ...
  for (let i = 1; i < dailyReturns.length; i++) {
    ...
    excessDaily.push(dailyReturns[i] - btcRet);
  }
  const bullCapture = btcUpSum > 0 ? botUpSum / btcUpSum : 0;

  // v7 EXCESS SORTINO — Sortino of daily excess returns (bot - BH)
  const excessMean = excessDaily.length > 0 ? excessDaily.reduce((a, b) => a + b, 0) / excessDaily.length : 0;
  const excessDownside = excessDaily.filter((r) => r < 0);
  const excessDownMean = excessDownside.length > 0 ? excessDownside.reduce((a, b) => a + b, 0) / excessDownside.length : 0;
  const excessDownStd = excessDownside.length > 1
    ? Math.sqrt(excessDownside.reduce((a, b) => a + (b - excessDownMean) ** 2, 0) / (excessDownside.length - 1))
    : 0.0001;
  const excessSortino = excessDownStd > 0 ? excessMean / excessDownStd * Math.sqrt(365) : 0;

  // Buy-hold CAGR for reporting
  const bhTotalReturn = buyHoldFinal / START_CAPITAL;
  const buyHoldCagr = years > 0 && bhTotalReturn > 0 ? Math.pow(bhTotalReturn, 1 / years) - 1 : -1;

  // Fitness placeholder; evalKFold recomputes with excess-Sortino aggregation
  const fitness = excessSortino;
  ...
  return {
    ...
    excess_sortino: excessSortino,
    excess_mean_daily: excessMean,
    buy_hold_cagr: buyHoldCagr,
  };
}
...
function evalKFold(bars: BtcDay[], indic: { rsi: number[]; vol20: number[] }, eventStates: EventState[], macroStates: MacroState[], genome: Genome, foldRanges: { start: number; end: number }[], trainStartIdx: number, trainEndIdx: number): { fitness: number; folds: FoldResult[]; concatRes: BotResult } {
  ...
  for (let k = 0; k < foldRanges.length; k++) {
    ...
    if (res.excess_sortino <= 0) anyNegExcess = true;
  }
  ...
  else if (anyNegExcess) fitness = 0; // v7: any fold with negative excess-Sortino kills fitness
  else {
    // v7: geometric mean of per-fold excess-Sortino (vs buy-hold)
    const prod = folds.reduce((a, f) => a * f.res.excess_sortino, 1);
    fitness = Math.pow(prod, 1 / folds.length);
  }
...
interface SeedResult {
  ...
  best_genome: Genome;
  fitness: number;
  fold_results: Array<{ fold: number; date_start: string; date_end: string; cagr: number; dd: number; calmar: number; bull_capture: number; excess_sortino: number; trades: number }>;
  train_calmar: number;
  train_cagr: number;
  train_dd: number;
  train_bull_capture: number;
  train_excess_sortino: number;
  val_calmar: number;
  val_cagr: number;
  val_dd: number;
  val_bull_capture: number;
  val_trades: number;
  val_excess_sortino: number; // v7: KEY metric for A/B comparison
  val_bh_cagr: number;
...
async function runOneSeed(
  ...
  // Watchdog state
  // Watchdog tracks excess-Sortino divergence in v7 (was Calmar in v6)
  const watchdogHistory: Array<{ gen: number; trainExcSort: number; valExcSort: number }> = [];
  ...
  while (Date.now() - startTime < durationMs) {
    ...
    if (gen % WATCHDOG_CHECK_EVERY_GENS === 0 && bestEver) {
      ...
      watchdogHistory.push({ gen, trainExcSort: trainRes.excess_sortino, valExcSort: valRes.excess_sortino });
      if (watchdogHistory.length >= WATCHDOG_TRIP_AFTER_DECLINES + 1) {
        ...
        for (let i = 1; i < recent.length; i++) {
          if (recent[i].valExcSort < recent[i - 1].valExcSort) valDeclines++;
          if (recent[i].trainExcSort > recent[i - 1].trainExcSort) trainClimbs++;
        }
  ...
  return {
    ...
    fold_results: kFinal.folds.map((f) => ({
      ...
      excess_sortino: f.res.excess_sortino,
    ...
    train_excess_sortino: trainRes.excess_sortino,
    ...
    val_excess_sortino: valRes.excess_sortino,
    val_bh_cagr: valRes.buy_hold_cagr,
...
async function main() {
  const runId = process.argv[2] ?? "v7";
  const minutesPerSeed = Number(process.argv[3] ?? 2);
  const nSeeds = Number(process.argv[4] ?? 20);
  const armsFilter = (process.argv[5] ?? "A").toUpperCase(); // v7 defaults to A only
  const seedBase = Number(process.argv[6] ?? 3000);            // v7 fresh seed range
  ...
  console.log(`Argus v7 (excess-Sortino fitness) — run ${runId}, ${minutesPerSeed}min/seed × ${nSeeds}seeds × ${armsToRun.length} arms (${armsToRun.join(",")}) = ${(minutesPerSeed * nSeeds * armsToRun.length).toFixed(0)}min total, seedBase=${seedBase}\n`);
  ...
  for (const arm of armsToRun) {
    ...
    for (const seed of armSeeds[arm]) {
      ...
      console.log(
        ...
        `val ExcSort ${r.val_excess_sortino.toFixed(2).padStart(6)} | ` +
        `val Calmar ${r.val_calmar.toFixed(2).padStart(5)} | ` +
        `val CAGR ${(r.val_cagr * 100).toFixed(1).padStart(5)}% | ` +
        `BH ${(r.val_bh_cagr * 100).toFixed(1).padStart(5)}% | ` +
        `${wc}`
      );
    }
  }

  // v7: analysis on validation EXCESS-SORTINO (the new fitness metric)
  ...
  const summarize = (arr: SeedResult[], name: string) => {
    ...
    const vals = arr.map((r) => r.val_excess_sortino);
    const mean = vals.reduce((a, b) => a + b, 0) / vals.length;
    const median = [...vals].sort((x, y) => x - y)[Math.floor(vals.length / 2)];
    const best = arr.reduce((b, r) => r.val_excess_sortino > b.val_excess_sortino ? r : b, arr[0]);
    console.log(`Arm ${name}: n=${arr.length}  mean_ExcSort=${mean.toFixed(3)}  median=${median.toFixed(3)}  min=${Math.min(...vals).toFixed(3)}  max=${Math.max(...vals).toFixed(3)}`);
    console.log(`  Best-of-${name} (seed ${best.seed}): ExcSortino ${best.val_excess_sortino.toFixed(3)} | Calmar ${best.val_calmar.toFixed(3)} | CAGR ${(best.val_cagr*100).toFixed(1)}% (BH ${(best.val_bh_cagr*100).toFixed(1)}%) | DD ${(best.val_dd*100).toFixed(1)}% | trades ${best.val_trades}`);
    return { n: arr.length, mean, median, best };
  };

  console.log("\n" + "=".repeat(80));
  console.log("A/B RESULT — validation Excess-Sortino distributions (v7 key metric)");
  console.log("=".repeat(80));
  ...
  let pAdj = 1;
  if (sumA && sumB) {
    const aVals = armA.map((r) => r.val_excess_sortino);
    const bVals = armB.map((r) => r.val_excess_sortino);
