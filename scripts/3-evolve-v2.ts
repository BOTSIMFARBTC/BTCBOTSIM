/**
 * v2 — world-enriched, anti-cheat BTC bot evolution.
 *
 * Improvements over v1:
 *   1. TRAIN / TEST split. GA sees train only. Test reported at the end.
 *      Default split: 2010-08-18..2022-12-31 train, 2023-01-01..end test.
 *   2. CALMAR fitness (return / max DD) with hard cliff: DD > 40% => 0 fitness.
 *   3. INTRABAR stop/target: use bar low against stop, bar high against target,
 *      fill at trigger price. If both hit same bar, ASSUME STOP FIRST (pessimistic).
 *   4. NEXT-BAR-OPEN fills for entry decisions (1-bar latency, realistic).
 *   5. NEWS-AWARE state: bot sees (category, days_ago) for events in the last 60d.
 *      Bot does NOT see impact_score / magnitude — those are Argus's opinion (data-snooped).
 *      Bot must EVOLVE its own reactivity to each category via new genes.
 *   6. DETERMINISTIC seeded RNG so runs are reproducible.
 *   7. Halving-cycle awareness: bot sees days_since_halving / days_to_next_halving.
 *
 * Genome: 23 genes (v1's 15 + 5 news + 3 halving).
 *
 * CLI: npx tsx scripts/3-evolve-v2.ts <runId> <minutes> [seed]
 */
...
const GENOME_SIZE = 23;
const MAX_DD_CLIFF = 0.40; // DD above this => fitness = 0

// Train/test split (inclusive train_end)
...
function decode(g: Genome) {
  return {
    ...
    // v2 news reactivity (15..19). Each fear = position multiplier when event within memory window.
    // fear=0 ignores events, fear=1 exits fully on any recent event of that type.
    hack_fear: g[15],           // reaction to recent hack
    regulation_fear: g[16],     // reaction to recent regulation event
    macro_shock_fear: g[17],    // reaction to recent macro shock
    geopolitical_fear: g[18],   // reaction to recent war/sanction event
    news_memory_days: 5 + Math.floor(g[19] * 55), // 5-60 days
    // v2 halving cycle (20..22)
    halving_bull_days: 30 + Math.floor(g[20] * 470),   // 30-500 days post-halving are bull
    halving_bull_boost: 1 + g[21],                      // 1.0-2.0x position during bull window
    pre_halving_accum_days: Math.floor(g[22] * 90),     // 0-90 days pre-halving to accumulate
  };
}
...
function simulate(bars: BtcDay[], indic: { rsi: number[]; vol20: number[] }, eventStates: EventState[], genome: Genome, startIdx: number, endIdx: number): BotResult {
  ...
  for (let i = startIdx; i <= endIdx; i++) {
    ...
    if (pendingTargetPct !== null) {
      ...
      if (Math.abs(delta) > 0.05) {
        if (delta > 0) {
          ...
          const btcBought = (usdToBuy - cost) / priceOpen;
          cash -= usdToBuy;
          btcHeld += btcBought;
          if (btcHeld > 0 && entryPrice === 0) entryPrice = priceOpen;
          nTrades++;
        } else {
          const usdToSell = -delta * equityAtOpen;
          const btcToSell = usdToSell / priceOpen;
    ...
    if (btcHeld > 0) {
      ...
      if (exitPrice !== null) {
        const usdToSell = btcHeld * exitPrice;
        const cost = usdToSell * roundTripCost * 0.5;
        cash += usdToSell - cost;
        btcHeld = 0;
        entryPrice = 0;
    ...
    // --- 6. News-fear adjustment ---
    // For each category, if event within memory window, apply fear multiplier to reduce position
    const es = eventStates[i];
    const mem = p.news_memory_days;
    const newsMultiplier = (daysSince: number, fear: number): number => {
      if (daysSince >= mem) return 1.0;
      // Linear decay: fresh event = full fear, oldest in window = zero fear
      const freshness = 1 - daysSince / mem;
      return Math.max(0, 1 - fear * freshness);
    };
    let riskMult = 1;
    riskMult *= newsMultiplier(es.days_since_hack, p.hack_fear);
    riskMult *= newsMultiplier(es.days_since_regulation, p.regulation_fear);
    riskMult *= newsMultiplier(es.days_since_macro, p.macro_shock_fear);
    riskMult *= newsMultiplier(es.days_since_geopolitical, p.geopolitical_fear);

    // --- 7. Halving cycle adjustment ---
    let halvingBoost = 1;
    if (es.days_since_last_halving <= p.halving_bull_days) halvingBoost = p.halving_bull_boost;
    if (es.days_to_next_halving <= p.pre_halving_accum_days) halvingBoost = Math.max(halvingBoost, 1 + (p.halving_bull_boost - 1) * 0.5);

    // --- 8. Position sizing ---
    const volFactor = 1 - p.vol_scale * Math.max(0, vol - 0.02);
    const targetSize = Math.max(0, Math.min(1, p.base_position * volFactor * riskMult * halvingBoost));
...
async function main() {
  const runId = process.argv[2] ?? "v2";
  const durationMin = Number(process.argv[3] ?? 5);
  const seed = Number(process.argv[4] ?? 42);
  const durationMs = durationMin * 60 * 1000;
  const rng = makeRng(seed);

  console.log(`Argus v2 evolution — run ${runId}, budget ${durationMin} min, seed ${seed}\n`);
  ...
  while (Date.now() - startTime < durationMs) {
    ...
    for (const bot of pop) {
      if (bot.f === -Infinity) {
        const res = simulate(bars, indic, eventStates, bot.g, trainStartIdx, trainEndIdx);
        bot.f = res.fitness;
        if (!bestEver || bot.f > bestEver.f) bestEver = { g: bot.g, f: bot.f, result: res };
      }
    }
    pop.sort((a, b) => b.f - a.f);

    const bestBotResult = simulate(bars, indic, eventStates, pop[0].g, trainStartIdx, trainEndIdx);
    genLog.push({
      gen, best_fit: Number(pop[0].f.toFixed(4)),
      median_fit: Number(pop[Math.floor(POP_SIZE / 2)].f.toFixed(4)),
      best_calmar: Number(bestBotResult.calmar.toFixed(3)),
      best_cagr_pct: Number((bestBotResult.cagr * 100).toFixed(2)),
      best_dd_pct: Number((bestBotResult.max_dd_pct * 100).toFixed(2)),
    ...
    if (gen % 5 === 1 || gen < 3) {
      console.log(
        ...
        `CAGR ${(bestBotResult.cagr * 100).toFixed(1).padStart(6)}% | ` +
        `DD ${(bestBotResult.max_dd_pct * 100).toFixed(1).padStart(5)}% | ` +
        `Calmar ${bestBotResult.calmar.toFixed(2).padStart(6)} | ` +
        `trades ${bestBotResult.n_trades.toString().padStart(4)} | ` +
        `${bestBotResult.liquidated ? "☠" : "✓"}`
      );
    }
  ...
  // Re-eval best bot on train AND test slices for final report
  ...
  console.log("\n" + "=".repeat(80));
  console.log("BEST BOT — TRAIN slice (GA saw this)");
  console.log("=".repeat(80));
  console.log(`Fitness (Calmar w/ DD cliff): ${bestEver.f.toFixed(3)}`);
  ...
  console.log(`  News fear hack ${p.hack_fear.toFixed(2)} reg ${p.regulation_fear.toFixed(2)} macro ${p.macro_shock_fear.toFixed(2)} geo ${p.geopolitical_fear.toFixed(2)} mem ${p.news_memory_days}d`);
  console.log(`  Halving bull ${p.halving_bull_days}d boost ${p.halving_bull_boost.toFixed(2)}x accum ${p.pre_halving_accum_days}d`);
  ...
  await fs.writeFile(path.join(RESULTS_DIR, `${runId}-best-bot.json`), JSON.stringify({
    ...
    fitness_function: "calmar_with_dd_cliff",
