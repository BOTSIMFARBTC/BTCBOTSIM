/**
 * v3 — Step A: reduce genome flexibility. Anti-cheat env unchanged from v2.
 *
 * Motivation: v2 (23 genes) overfit — train CAGR 297.9% -> test CAGR 1.9%.
 * Hypothesis: too many per-category fear genes + halving-boost genes let the GA
 * memorize specific historical events. Shrink genome to reduce degrees of freedom.
 *
 * Same anti-cheat as v2 (train/test split, Calmar+DD cliff, intrabar stops,
 * next-bar-open fills, seeded RNG).
 *
 * Changes vs v2:
 *   - Drop per-category fear genes (hack/reg/macro/geo) — 4 genes gone.
 *   - Drop halving-boost genes (bull_days, bull_boost, pre_accum) — 3 gone.
 *   - Add: 1 generic bearish_event_fear + 1 event_memory_days.
 *   - Net: 23 - 7 + 2 = 17 genes (v1's 15 + 2 event genes).
 *   - Single fear applies to ANY event in inherently-bearish categories:
 *     hack, macro, geopolitical, market_structure. Adoption/onchain/regulation
 *     are NOT reacted to (removes my labeling bias for ambiguous cases).
 *
 * CLI: npx tsx scripts/4-evolve-v3.ts <runId> <minutes> [seed]
 */
...
async function main() {
  const runId = process.argv[2] ?? "v3";
  const durationMin = Number(process.argv[3] ?? 5);
  const seed = Number(process.argv[4] ?? 42);
  const durationMs = durationMin * 60 * 1000;
  const rng = makeRng(seed);

  console.log(`Argus v3 evolution (17-gene reduced) — run ${runId}, budget ${durationMin} min, seed ${seed}\n`);
