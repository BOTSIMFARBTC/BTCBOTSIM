/**
 * v4 — Step B: K-fold fitness + realism patch #1 (stochastic slippage).
 *
 * v3 shrunk the genome and cut overfit 14× (test CAGR 1.9%→19.9%). But test
 * Calmar 0.68 still below 1.0 ship gate. Hypothesis: bot fits the specific
 * temporal ordering of the 12.4yr train slice. Force regime robustness.
 *
 * Same 17-gene genome as v3. Same anti-cheat env.
 *
 * Changes vs v3:
 *   A. K-fold fitness. Train slice split into 4 windows of ~3.1yr each.
 *      Bot simulated in each window independently starting fresh at $1000.
 *      Fitness = mean(Calmar per fold), hard cliff: if ANY fold DD > 40% or
 *      liquidates => fitness = 0. Kills strategies relying on a single BTC era.
 *
 *   B. Realism patch #1: slippage. base_slippage = 5 bps, vol_slippage adds
 *      200*max(0, vol20-0.02) bps during volatile bars. Deterministic (uses
 *      vol20 not random) so results reproducible with seed.
 *
 * CLI: npx tsx scripts/5-evolve-v4.ts <runId> <minutes> [seed]
 */
...
function evalKFold(bars: BtcDay[], indic: { rsi: number[]; vol20: number[] }, eventStates: EventState[], genome: Genome, foldRanges: { start: number; end: number }[]): { fitness: number; folds: FoldResult[] } {
  ...
  let fitness = 0;
  if (anyLiquidated) fitness = -100;
  else if (anyOverCliff) fitness = 0;
  else {
    const meanCalmar = folds.reduce((a, f) => a + f.res.calmar, 0) / folds.length;
    fitness = meanCalmar;
  }
  return { fitness, folds };
}

async function main() {
  const runId = process.argv[2] ?? "v4";
  const durationMin = Number(process.argv[3] ?? 10);
  const seed = Number(process.argv[4] ?? 42);
  const durationMs = durationMin * 60 * 1000;
  const rng = makeRng(seed);

  console.log(`Argus v4 evolution (K-fold + slippage) — run ${runId}, budget ${durationMin} min, seed ${seed}\n`);
  ...
  while (Date.now() - startTime < durationMs) {
    ...
    for (const bot of pop) {
      if (bot.f === -Infinity) {
        const kres = evalKFold(bars, indic, eventStates, bot.g, foldRanges);
        bot.f = kres.fitness;
    ...
    // Log per-fold best
    const kBest = evalKFold(bars, indic, eventStates, pop[0].g, foldRanges);
    genLog.push({
  ...
  const kBestFinal = evalKFold(bars, indic, eventStates, bestEver.g, foldRanges);
  ...
  await fs.writeFile(path.join(RESULTS_DIR, `${runId}-best-bot.json`), JSON.stringify({
    ...
    fitness_function: "kfold_mean_calmar_with_percut_dd_cliff",
