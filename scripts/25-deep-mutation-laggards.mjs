/**
 * Deep 4-generation mutation focused on the 3 lagging bots.
 * σ decays 0.08 → 0.05 → 0.03 → 0.02. 40 mutants per gen.
 * WR-priority fitness: WR × 100 + exp_lo / 10
 * Also allows crossover with any top-2 alpha bot each generation.
 */
...
function crossover(genA, genB, seed) {
  const rng = mulberry32(seed); const len = Math.min(genA.length, genB.length);
  const child = new Array(len);
  for (let i = 0; i < len; i++) { const base = rng() < 0.5 ? genA[i] : genB[i]; child[i] = Math.max(0, Math.min(1, base + gaussian(rng) * 0.02)); }
  return child;
}
...
async function evaluate(genome, label) {
  ...
  const memberCfg = { address: `0xDP_${label}`, bot_id: bot.id, execution_mode: "independent", autoMode: "MIRROR", minConfidence: 0, allowedDirection: "BULL_ONLY", maxLeverage: 1, maxNotionalUsdc: 500, maxDailyTrades: 999, maxConcurrent: 1, sizingMode: "PCT_OF_CAPITAL", sizingParam: 0.5 };
...
async function main() {
  const N_GENS = 4, N_MUTANTS = 30;
  const SIGMAS = [0.08, 0.05, 0.03, 0.02];
  const MIN_POS_COH = 76;
  const FITNESS_MIN_GAIN = 30;

  const pg = JSON.parse(readFileSync(PG_PATH, "utf8"));
  const parents = pg.top5;
  // Alphas for crossover: top 2 by fitness
  const laggards = parents.filter(p => p.rank >= 3);
  const alphas = parents.filter(p => p.rank <= 2);
  console.log(`\n═══ DEEP MUTATION + CROSSOVER ON LAGGARDS ═══`);
  console.log(`Laggards: ${laggards.map(l => l.label).join(", ")}`);
  console.log(`Alphas (for xover): ${alphas.map(a => a.label).join(", ")}`);
  ...
  for (const lag of laggards) {
    console.log(`\n╔ Improving ${lag.label} ═════`);
    let currentGenome = lag.genome, currentLabel = lag.label;
    let currentEval = await evaluate(currentGenome, `orig-${lag.label}`);
    console.log(`  baseline: WR ${currentEval.wr.toFixed(1)}%  fit ${currentEval.fitness.toFixed(0)}  pos ${currentEval.posCohorts}/77`);

    for (let g = 0; g < N_GENS; g++) {
      const sigma = SIGMAS[g];
      let best = null;
      for (let m = 0; m < N_MUTANTS; m++) {
        const seed = (`${lag.label}_g${g}_m${m}`).split("").reduce((a, c) => (a * 31 + c.charCodeAt(0)) >>> 0, 0);
        const mutant = mutate(currentGenome, sigma, seed);
        const ev = await evaluate(mutant, `${lag.label}_g${g}_m${m}`);
        if (ev.posCohorts < MIN_POS_COH) continue;
        if (ev.fitness > currentEval.fitness + FITNESS_MIN_GAIN && (!best || ev.fitness > best.eval.fitness)) {
          best = { genome: mutant, eval: ev, kind: 'mut', idx: m };
        }
      }
      // Also try crossover with each alpha
      for (const alpha of alphas) {
        for (let x = 0; x < 15; x++) {
          const seed = (`${lag.label}_g${g}_x${alpha.label}_${x}`).split("").reduce((a, c) => (a * 31 + c.charCodeAt(0)) >>> 0, 0);
          const child = crossover(currentGenome, alpha.genome, seed);
          const ev = await evaluate(child, `${lag.label}_g${g}_x_${alpha.label.slice(0,10)}_${x}`);
          if (ev.posCohorts < MIN_POS_COH) continue;
          if (ev.fitness > currentEval.fitness + FITNESS_MIN_GAIN && (!best || ev.fitness > best.eval.fitness)) {
            best = { genome: child, eval: ev, kind: `x-${alpha.label.slice(0,10)}`, idx: x };
          }
        }
      }
      if (best) {
        currentGenome = best.genome;
        currentLabel = `${lag.label}-g${g}-${best.kind}${best.idx}`;
        currentEval = best.eval;
        console.log(`  gen${g+1} σ=${sigma}: ADOPTED ${best.kind}${best.idx}  WR ${currentEval.wr.toFixed(1)}%  fit ${currentEval.fitness.toFixed(0)}  pos ${currentEval.posCohorts}/77`);
      } else {
        console.log(`  gen${g+1} σ=${sigma}: no improvement`);
      }
    }
    results.push({ orig: lag, finalGenome: currentGenome, finalLabel: currentLabel, finalEval: currentEval, improved: currentLabel !== lag.label });
  }
  console.log(`\nElapsed: ${((Date.now() - t0) / 1000).toFixed(1)}s`);

  const improvedCount = results.filter(r => r.improved).length;
  if (improvedCount === 0) { console.log("No improvements."); return; }

  console.log(`\n${improvedCount}/3 laggards improved. Writing updated genomes.`);
  const newTop5 = parents.map(p => {
    const res = results.find(r => r.orig.label === p.label);
    if (res && res.improved) {
      return { label: res.finalLabel, val_calmar: p.val_calmar, genome: res.finalGenome, rank: p.rank, sizeMultiplier: p.sizeMultiplier };
    }
    return p;
  });
  writeFileSync(PG_PATH, JSON.stringify({ ...pg, session8_deep_laggards: true, top5: newTop5 }, null, 2));
  console.log(`Wrote portfolio-genomes.json.`);
}
