/**
 * v5 — Step C: macro state + geo-mean fitness + realism patch #2.
 *
 * Motivation: v3 & v4b are one-genome-fits-all-eras. v3 gets OOS Calmar 0.68
 * because its train era was bull-heavy (matching OOS regime); v4b gets 0.22
 * because it's tuned for bear survival too. Both bots average across regimes
 * rather than react to conditions. Solution: give the bot macro state so it
 * can CONDITION on regime instead of averaging.
 *
 * Changes vs v4b:
 *   A. MACRO STATE. Bot sees percentile-normalized VIX / DXY / ^TNX / SP500 20d ret.
 *      Percentile is rolling 250d — regime-relative, not level-dependent.
 *   B. GENOME +4 macro genes: vix_fear, dxy_fear, yield_fear, riskon_boost.
 *      Total 21 genes (17 + 4).
 *   C. FITNESS: geometric mean of Calmars across folds (middle ground between
 *      min-Calmar's over-conservatism and mean-Calmar's gaming). Multiplicative
 *      penalty for low folds, symmetric reward for high folds.
 *   D. REALISM PATCH #2: event visibility delay 1 day (bot sees events day-after,
 *      not same-day — more realistic reaction time for daily-cadence trader).
 *   E. PER-REGIME OOS REPORT: break OOS test into bull/chop/bear buckets and
 *      report bot's contribution per regime. This tells us WHETHER macro state
 *      actually let the bot ride regimes vs blur across them.
 *
 * All prior anti-cheat preserved: OOS split, intrabar stops, next-bar-open,
 * blind events, deterministic seed, slippage 5+200*vol bps.
 *
 * CLI: npx tsx scripts/8-evolve-v5.ts <runId> <minutes> [seed]
 */
...
const GENOME_SIZE = 21; // v4b's 17 + 4 macro genes
const MAX_DD_CLIFF = 0.40; // DD above this => fitness = 0 (per-fold too)
const EVENT_DELAY_DAYS = 1; // v5 realism patch #2: bot sees events N days after they happen
const MACRO_PCTILE_WINDOW = 250; // rolling window for percentile normalization
...
function decode(g: Genome) {
  return {
    ...
    // v3 unified event reactivity (15..16)
    bearish_event_fear: g[15],
    event_memory_days: 5 + Math.floor(g[16] * 55), // 5-60 days
    // v5 macro reactivity (17..20). Thresholds fixed at 0.8 pctile so genome
    // only controls REACTION strength, not detection level (reduces overfit).
    macro_vix_fear: g[17],       // reduce pos when VIX pctile > 0.8
    macro_dxy_fear: g[18],       // reduce pos when DXY pctile > 0.8 (strong dollar bad for BTC)
    macro_yield_fear: g[19],     // reduce pos when 10Y pctile > 0.8 (rising rates)
    macro_riskon_boost: g[20],   // boost pos when SP500 20d up + VIX pctile < 0.3
  };
}
...
function simulate(bars: BtcDay[], indic: { rsi: number[]; vol20: number[] }, eventStates: EventState[], macroStates: MacroState[], genome: Genome, startIdx: number, endIdx: number): BotResult {
  ...
  for (let i = startIdx; i <= endIdx; i++) {
    ...
    // --- 7. Macro reactivity (v5) ---
    const ms = macroStates[i];
    let macroMult = 1;
    if (ms.vix_pctile > 0.8) macroMult *= Math.max(0, 1 - p.macro_vix_fear * (ms.vix_pctile - 0.8) / 0.2);
    if (ms.dxy_pctile > 0.8) macroMult *= Math.max(0, 1 - p.macro_dxy_fear * (ms.dxy_pctile - 0.8) / 0.2);
    if (ms.us10y_pctile > 0.8) macroMult *= Math.max(0, 1 - p.macro_yield_fear * (ms.us10y_pctile - 0.8) / 0.2);
    // Risk-on boost when SP500 recent uptrend + VIX suppressed
    let macroBoost = 1;
    if (ms.sp500_20d_ret > 0.02 && ms.vix_pctile < 0.3) {
      macroBoost = 1 + p.macro_riskon_boost * 0.5;
    }

    // --- 8. Position sizing ---
    const volFactor = 1 - p.vol_scale * Math.max(0, vol - 0.02);
    const targetSize = Math.max(0, Math.min(1, p.base_position * volFactor * riskMult * macroMult * macroBoost));
...
function evalKFold(bars: BtcDay[], indic: { rsi: number[]; vol20: number[] }, eventStates: EventState[], macroStates: MacroState[], genome: Genome, foldRanges: { start: number; end: number }[], trainStartIdx: number, trainEndIdx: number): { fitness: number; folds: FoldResult[]; concatRes: BotResult } {
  ...
  else if (anyNegCalmar) fitness = 0; // geo-mean requires all positive
  else {
    // v5: GEOMETRIC MEAN of Calmars — middle ground between min and mean.
    // Multiplicative penalty for low folds, symmetric reward for high folds.
...
async function main() {
  const runId = process.argv[2] ?? "v5";
  const durationMin = Number(process.argv[3] ?? 10);
  const seed = Number(process.argv[4] ?? 42);
  const durationMs = durationMin * 60 * 1000;
  const rng = makeRng(seed);

  console.log(`Argus v5 (macro + geo-mean K-fold + event delay + slippage) — run ${runId}, budget ${durationMin} min, seed ${seed}\n`);
  ...
  while (Date.now() - startTime < durationMs) {
    ...
    // Log per-fold best
    const kBest = evalKFold(bars, indic, eventStates, macroStates, pop[0].g, foldRanges, trainStartIdx, trainEndIdx);
    genLog.push({
  ...
  const kBestFinal = evalKFold(bars, indic, eventStates, macroStates, bestEver.g, foldRanges, trainStartIdx, trainEndIdx);
  const trainRes = simulate(bars, indic, eventStates, macroStates, bestEver.g, trainStartIdx, trainEndIdx);
  const testRes = simulate(bars, indic, eventStates, macroStates, bestEver.g, testStartIdx, testEndIdx);
  ...
  console.log("\n" + "=".repeat(80));
  console.log("OOS PER-REGIME BREAKDOWN (bot daily returns bucketed by 60d BTC context)");
  console.log("=".repeat(80));
  const buckets: Record<string, { days: number; sumRet: number; sumBtcRet: number }> = {
    "strong bull (>20%/60d)": { days: 0, sumRet: 0, sumBtcRet: 0 },
    "mild bull (5-20%/60d)": { days: 0, sumRet: 0, sumBtcRet: 0 },
    "chop (-5..+5%/60d)": { days: 0, sumRet: 0, sumBtcRet: 0 },
    "mild bear (-20..-5%/60d)": { days: 0, sumRet: 0, sumBtcRet: 0 },
    "strong bear (<-20%/60d)": { days: 0, sumRet: 0, sumBtcRet: 0 },
  };
  const closes = bars.map((b) => b.close);
  for (let i = 0; i < testRes.daily_returns.length; i++) {
    const testIdx = testStartIdx + i;
    if (testIdx < 60) continue;
    const btc60d = (closes[testIdx] - closes[testIdx - 60]) / closes[testIdx - 60];
    let key: string;
    if (btc60d > 0.20) key = "strong bull (>20%/60d)";
    else if (btc60d > 0.05) key = "mild bull (5-20%/60d)";
    else if (btc60d > -0.05) key = "chop (-5..+5%/60d)";
    else if (btc60d > -0.20) key = "mild bear (-20..-5%/60d)";
    else key = "strong bear (<-20%/60d)";
    const b = buckets[key];
    b.days++;
    b.sumRet += testRes.daily_returns[i];
    b.sumBtcRet += btc60d / 60;
  }
  for (const [k, v] of Object.entries(buckets)) {
    if (v.days === 0) { console.log(`  ${k.padEnd(28)} : 0 days`); continue; }
    const botAvg = v.sumRet / v.days;
    const btcAvg = v.sumBtcRet / v.days;
    console.log(`  ${k.padEnd(28)} : ${v.days.toString().padStart(3)}d | bot avg ${(botAvg*100).toFixed(3)}%/d | BTC avg ${(btcAvg*100).toFixed(3)}%/d`);
  }
  ...
  console.log(`  Macro fear vix ${p.macro_vix_fear.toFixed(2)} dxy ${p.macro_dxy_fear.toFixed(2)} yield ${p.macro_yield_fear.toFixed(2)} | risk-on boost ${p.macro_riskon_boost.toFixed(2)}`);
  ...
  await fs.writeFile(path.join(RESULTS_DIR, `${runId}-best-bot.json`), JSON.stringify({
    ...
    fitness_function: "kfold_geomean_calmar_with_percut_and_concat_dd_cliff",
    genome_size: GENOME_SIZE,
    event_delay_days: EVENT_DELAY_DAYS,
    macro_pctile_window: MACRO_PCTILE_WINDOW,
